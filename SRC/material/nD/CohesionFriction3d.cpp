...
/*
Source: /DEVELOPER/element/cpp/Macroelement3d/Macroelement3d/CohesionFriction3d.cpp
...
static int numCohesionFriction3d = 0;

void *OPS_CohesionFriction3d()
{
  // print out some KUDO's
  if (numCohesionFriction3d == 0) {
    opserr << "CohesionFriction3d - Loaded from external library. Written by Igor Tomic, EPFL, 2019.\n";
    numCohesionFriction3d =1;
  }
  ...
  if (OPS_GetIntInput(&numData, &iData[0]) < 0) {
	  opserr << "WARNING: invalid tag. Required structure: ndMaterial CohesionFriction3d $tag $G $mu $c $GfII <-flags> " << endln;
	  return 0;
  }
  ...
  if (OPS_GetDoubleInput(&numData, &dData[0]) < 0) {
	  opserr << "WARNING: invalid double inputs. Required structure: ndMaterial CohesionFriction3d $tag $G $mu $c $GfII<-flags> " << endln;
	  return 0;
  }
  ...
  if (strcmp(inputStructure, "-imposedSigma") == 0) {
	  ...
	  if (OPS_GetDoubleInput(&numData, &dData2[0]) < 0) {
		  opserr << "WARNING: invalid double inputs. Required structure: ndMaterial CohesionFriction3d $tag $G $mu $c $GfII -imposedSigma $sigma0 " << endln;
		  return 0;
	  }
	  imposedSigma = true;
	  sigma0 = dData2[0];

	  theMaterial = new CohesionFriction3d(iData[0], dData[0], dData[1], dData[2], dData[3], sigma0);
	  return theMaterial;
  }
  else {
	  if (strcmp(inputStructure, "-axialMaterial") == 0) {
		  ...
		  if (OPS_GetIntInput(&numData, &iData2[0]) < 0) {
			  opserr << "WARNING: invalid integer inputs. Required structure: ndMaterial CohesionFriction3d $tag $G $mu $c $GfII -axialMaterial $axialMaterialTag " << endln;
			  return 0;
		  }
		  ...
		  theMaterial = new CohesionFriction3d(iData[0], theAxialModel, dData[0], dData[1], dData[2], dData[3]);
		  return theMaterial;
	  }
  }
 
  if (theMaterial == 0) {
    opserr << "WARNING could not create NDMaterial of type CohesionFriction3d\n";
    return 0;
  }
...
// full constructors
CohesionFriction3d::CohesionFriction3d(int tag, UniaxialMaterial* comprMat, double G, double mu, double c, double GfII)
...
{ 
	...
	if (this->GfII < 1.1*GfII_lim) {
		...
		opserr << "CohesionFriction3d::tag:" << tag << ": corrected GfII (fracture energy in modeII) to " << this->GfII << " to avoid snap-back at material level." << endln;
	}

}


CohesionFriction3d::CohesionFriction3d(int tag, double G, double mu, double c, double GfII, double sigma0)
	:NDMaterial(tag, 0), stress(3), stressCommitted(3), u(3), uCommitted(3), up(3), upCommitted(3), Kpen(3, 3), K(3, 3), KCommitted(3, 3), theAxialMat(0), mu(mu), c(c), G(G), GfII(GfII),
	k(0.0), kCommitted(0.0),deltaLambda(0.0), imposedSigma(true), sigma0(sigma0)
{
	...
	if (sigma0 > 0.0) {
		sigma0 = -sigma0;
		opserr << "CohesionFriction3d::tag:" << tag << ": the constant vertical stress sigma0 is assumed to act in compression." << endln;
	}
...
CohesionFriction3d::CohesionFriction3d()
...
double 
CohesionFriction3d::hardeningLaw(double kk) {
	return c*exp(-c/GfII*kk);
}

double
CohesionFriction3d::der_hardeningLaw(double kk) {
	return -1.0*c*c/GfII *exp(-c / GfII*kk);
}

// standard methods
int 
CohesionFriction3d::setTrialStrain(const Vector &strain, const Vector &rate) {
	return this->setTrialStrain(strain);
}

int
CohesionFriction3d::setTrialStrain(const Vector &strain) { 
	...
	if (sigma0 > 0) {
		sigma0 = 0;
	}   // Also just for trial, rather comment later
	...
	if (f <= tol) {
		...
		//K(0,0)= axialStiffness; //Added just for trial, probably redundant and can be deleted.
		int maxIter = 50;
		...
		if (iter >= maxIter) {
			opserr << "CohesionFriction3d: did not converge in the return mapping." << endln;
			return -1;
		}
	...
	if (imposedSigma) {
		...
		stress(0) = theAxialMat->getStress();
		K(0, 0) = theAxialMat->getTangent(); // Not necessary, just for trial
	}
...
int
CohesionFriction3d::setTrialStrainIncr (const Vector &strain)
{
	opserr << "WARNING: CohesionFriction3d has no method setTrialStrainIncr implemented" << endln; 
	return 0;
}

int
CohesionFriction3d::setTrialStrainIncr (const Vector &strain, const Vector &rate)
{
  return this->setTrialStrainIncr(strain);
}

const Matrix& 
CohesionFriction3d::getCommittedTangent( )  {
	return KCommitted;
}
	
const Matrix& 
CohesionFriction3d::getTangent( ) 
{
	// use a fake small stiffness if zero stiffness
	double factor = 0.0000000001;

	K(0, 0) = theAxialMat->getTangent();
...
const Matrix&
CohesionFriction3d::getInitialTangent (void)
{ 
	return Kpen;
}

const Vector&
CohesionFriction3d::getStress (void)
{
	return stress;
}

const Vector&
CohesionFriction3d::getStrain (void)
{
  return u;
}

int
CohesionFriction3d::commitState (void)
{
	if (theAxialMat !=0)
...
int
CohesionFriction3d::revertToLastCommit (void)     
{ 
	if (theAxialMat != 0)
...
int
CohesionFriction3d::revertToStart (void)
{ 
	if (theAxialMat != 0)
...
NDMaterial*
CohesionFriction3d::getCopy (void)
{
	CohesionFriction3d *theCopy = 0;

	if (!imposedSigma)
		theCopy = new CohesionFriction3d(this->getTag(), theAxialMat, G, mu, c, GfII);
	else
		theCopy = new CohesionFriction3d(this->getTag(), G, mu, c, GfII, sigma0);

  return theCopy;
}

NDMaterial*
CohesionFriction3d::getCopy (const char *type) {
  return this->getCopy();
}

const char*
CohesionFriction3d::getType (void) const
{
  return "Interface3D";
}

int
CohesionFriction3d::getOrder (void) const
{
  return 3;
}
...
void 
CohesionFriction3d::Print( OPS_Stream &s, int flag )
{
  s << endln ;
  s << "CohesionFriction3d : " ; 
  s << this->getType( ) << endln ;
  s << endln ;
}

int 
CohesionFriction3d::sendSelf(int commitTag, Channel &theChannel) 
{  return -1;  }

int 
CohesionFriction3d::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{  return -1;  }
...
Response*
CohesionFriction3d::setResponse (const char **argv, int argc, OPS_Stream &output) {
...
int 
CohesionFriction3d::getResponse (int responseID, Information &matInfo)
{
